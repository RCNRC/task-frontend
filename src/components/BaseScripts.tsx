...
export function let_visible_to_admin(){
    let visibles = ["create_new_plan_button", "get_all_plans_button", "get_report", "buttons_for_creation_plan_field", "canvas_field"];
...
export function let_visible_to_user(){
    ...
    let hiddens = ["create_new_plan_button", "buttons_for_creation_plan_field"];
    visibles.forEach((class_name: string)=>{
        d3.select(`.${class_name}`).style("visibility", "visible");
    });
...
export function let_visible_to_none(){
    let hiddens = ["create_new_plan_button", "get_all_plans_button", "get_report", "buttons_for_creation_plan_field", "canvas_field"];
    hiddens.forEach((class_name: string)=>{
        d3.select(`.${class_name}`).style("visibility", "hidden");
    });
}